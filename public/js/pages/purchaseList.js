...
const createListRow = (ingredientInfo, ingredientIndex) => {
	...
	const checkInput = htmlCreator.createInput({
		type: "checkbox",
		id: ingredientInfo.name,
		className: "check-input-purchase-list",
	});
...
const createCentralContainer = (listInfos) => {
	const imgList = htmlCreator.createImg(
		"./assets/images/Checklist-pana.svg",
		"list-img"
	);
	const table = createListTable(listInfos);
	const centralContainer = htmlCreator.createDiv(
		"central-container-purchase"
	);
...
const createDivBtn = async (igredientsToDownload, eventID) => {
	...
	if (igredientsToDownload) {
		...
		const saveIcon = htmlCreator.createImg(
			"./assets/icons/save-purchase-icon.svg"
		);
		const buttonUpdateList = htmlCreator.createButton(
			"Salvar lista",
			"btn-save-list",
			"btn-section"
		);
		...
		const downloadBtn = htmlCreator.createButton(
			"Download",
			"download-btn",
			"btn-section"
		);
		...
		buttonUpdateList.addEventListener("click", async () => {
			...
			const checkboxIngredient = document.querySelectorAll(
				".check-input-purchase-list"
			);
			...
			const updateRequested = await updatePurchaseList(
				eventID,
				requestBody
			);
...
const createEmptyContentDiv = () => {
	...
	const title = htmlCreator.createTitle(
		"h2",
...
const createPurchaseListPage = async (constructorInfo = { eventID: "" }) => {
	...
	const divBtn = await createDivBtn(
		haveIngredientsList,
