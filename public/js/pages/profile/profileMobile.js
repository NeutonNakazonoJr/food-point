...
const createProfileMobile = () => {
	...
	async function getImageMenu() {
		...
		fetch("/api/upload")
			...
			.catch((error) => {
				notification(
					"Erro inesperado ao carregar a imagem de perfil, tente novamente mais tarde!"
				);
	...
	subtitle.textContent =
		"Em que podemos te ajudar? Entre em contato conosco! ";
	const helpIntr = document.createElement("p");
	helpIntr.className = "p";
	helpIntr.textContent =
		"Abaixo listamos as nossas formas de contato disponiveis. Estamos ansiosos em lhe atender da melhor maneira possivel.";
	const emails = document.createElement("div");
	emails.id = "help-mails";
	const topic = document.createElement("h3");
	topic.className = "help-topic";
	topic.textContent =
		"Entre em contato conosco através de um dos endereços de email abaixo: ";
	...
	repositoryTitle.textContent =
		"Quer saber mais sobre o nosso projeto? Visite nosso repositorio no GitHub: ";
	const respositoryAddress = document.createElement("a");
	respositoryAddress.id = "repository-link";
	respositoryAddress.textContent =
		"https://github.com/NeutonNakazonoJr/food-point";
