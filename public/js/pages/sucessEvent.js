...
const createSuccessEventPage = (constructorInfo = { event: { id: "" } }) => {
	...
	const headerSuccessEvent = getHeader(false, false);
	...
	h3Second.textContent = "Veja o seu evento agora ou vá para página inicial";
	...
	homePageButton.addEventListener("mouseover", () => {
		const imgHomePageButton2 = document.getElementById(
			"image-home-button-success"
		);
	...
	inviteEventButton.addEventListener("click", (e) => {
		e.preventDefault();
			dispatchOnStateChange("/event", {
				eventID: constructorInfo.event.id,
			});
	});
